...
)
{
   ...
   */
   ...
   //--------------------------------------------------
   // JDF edit:
   // create temporaries like in gbart
   double *z = new double[n];
   std::vector<double> sign(n);
  

   // Rcpp::Rcout << "yv = " << yv << std::endl;
   for (size_t i = 0; i < n; i++) {
     ...
     // Rcpp::Rcout << "sign[" << i << "] = " << sign[i] << std::endl;
     z[i] = sign[i];
     // Rcpp::Rcout << "z[" << i << "] = " << z[i] << std::endl;
   ...
   pi.mybeta=mybeta;
   pi.tau=tau;
   pi.sigma = 1.0; // JDF edit as per gbart

   //***** discrete prior for constrained model
   ...
   for(size_t i=0;i<n;i++){
      allfit[i]=ybar;
     // Rcpp::Rcout << "allfit[" << i << "] = " << allfit[i] << std::endl;
   }
   ...
   for(size_t i=0;i<(nd+burn);i++) {
      ...
      for(size_t j=0;j<m;j++) {
         fit(t[j],xi,di,ftemp);
         // Rcpp::Rcout << "inside j loop" << std::endl;
         // Rcpp::Rcout << "before k loop" << std::endl;
         for(size_t k=0;k<n;k++) {
           // Rcpp::Rcout << "inside k loop" << std::endl;
           // Rcpp::Rcout << "before allfit[" << k << "] = " << allfit[k] << std::endl;
           allfit[k] = allfit[k]-ftemp[k];
           // Rcpp::Rcout << "after allfit[" << k << "] = " << allfit[k] << std::endl;
           r[k] = z[k]-allfit[k]; //JDF EDIT
           // r[k] = y[k]-allfit[k];
           // Rcpp::Rcout << "z[" << k << "] = " << z[k] << std::endl;
           // Rcpp::Rcout << "r[" << k << "] = " << r[k] << std::endl;

           /*
            * here
            */
         }
         // Rcpp::Rcout << "after k loop" << std::endl;
      ...
      //draw sigma
/* JDF update: sigma is not part of probit regression
      rss=0.0;
...
*/
      ...
      // JDF update:
      for(size_t k=0;k<n;k++) {
        	  z[k]=sign[k]*rtnorm(sign[k]*allfit[k], -sign[k]*offset, 1.0, gen);  
      }
      ...
      if(i>=burn) {
         ...
         // Rprintf("trcnt,nkeeptrain,n: %d, %d,%d\n",trcnt,nkeeptrain,n);
         // Rprintf("trdraw(trcnt,5):%d\n",trdraw(trcnt,5));
